...
def olla_lut_simulation_quant_single_trail(num_time_slots:int, cqi_intervals:int, interval, input_batch_size:int, num_quant_bits:int):
    ...
    quantizer = UniformQuantizer(min=6, max=12, num_bits=num_quant_bits)

    modu_order = 2
    code_rate = 602/1024

    sinr_rnn_model = load_model("/home/zhu/Codes/link_adaptation/SINR_RNN_models/LSTM_SINR_prediction_input_20_output_5_new.h5")
    ...
    for time_index in range(num_time_slots):
        ...
        eff_sinr_quant = quantizer(eff_sinr_from_link)
        eff_sinr_link_list.append(eff_sinr_quant)
...
def olla_LUT_simulation_sinr_feedback(num_trails, num_time_slots, feedback_interval, sinr_prediction_input_dimension, num_bits):
    ...
    for idx_train in range(num_trails):
        ack_list_single_trail, tbs_list_single_trail, sinr_feedback_list, mcs_list_single_trail, sinr_list_single_trail = \
            olla_lut_simulation_quant_single_trail(num_time_slots, feedback_interval, sinr_prediction_input_dimension, num_bits)
    ...
    file_path = "results/new_olla_LUT_sinr_interval_" + str(feedback_interval) +"_lstm_quant_bits" + str(num_bits) \
...
if __name__ == "__main__":
    olla_LUT_simulation_sinr_feedback(num_trails=100, num_time_slots=500, feedback_interval=5, \
         sinr_prediction_input_dimension=20, num_bits=3)
