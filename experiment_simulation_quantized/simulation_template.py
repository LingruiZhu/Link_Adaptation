...
from la_agent.olla_agent_bp_non_linear import NonLinear_OLLA_bp
...
def generate_file_name(agent_name:str, num_trails:int, cqi_interval:int, num_quant_bits:int, cqi_delay:int, \
                    ...
                    variant_sinr:bool, gamma:float):
    file_name = f"{agent_name}_ack_weight_{ack_weight}_num_time_slots_{q_network_input_time_dimension}_num_trails_{num_trails}_cqi_interval_{cqi_interval}_quant_bits_{num_quant_bits}_delay_{cqi_delay}_add_interference_{add_interference}_variant_sinr_{variant_sinr}_gamma_{gamma}_{simulation_name}"
    return file_name
...
def olla_simulation_single_trail(olla_agent:OLLA_DQN_agent or OuterLoopLinkAdaptation or OLLA_DQN_agent_simple,
                                ...
                                variant_sinr:bool=False, trail_index:int=0):
    ...
    lut_link_env = LutEnvironment(ebno_db=12, variant_sinr=variant_sinr)
    lut_link_env.set_interference_mode("time")
    interference1 = InterferenceToy(tx_power=1, h_factor=0.005, interval=10, period=2)
    interference2 = InterferenceToy(tx_power=1, h_factor=0.002, interval=15, period=3)
    if add_interference:
    ...
    for time_index in range(num_time_slots):        
        ...
        if len(ack_list)>1:
            if olla_agent.__class__.__name__ == "OLLA_DQN_agent" or olla_agent.__class__.__name__  ==  "OLLA_DQN_agent_simple":
                ...
                #               sinr_new=eff_sinr,
                #               ack_new=ack)
                olla_agent.add_memory_only_sinr_offset(sinr=real_sinr_list[-2],
                              ack=ack_list[-1],
                              mcs_index=mcs_index,
                              action = action_index,
                              sinr_new=real_sinr_list[-1],
                              ack_new=ack)
        ...
        if olla_agent.__class__.__name__ == "OLLA_DQN_agent":
            ...
            mcs_index, code_rate, modu_order, sinr_adjusted, sinr_offset, estimate_bler = olla_agent.determine_mcs_action_from_sinr(sinr_feedback, ack)
            action_index = 0
        ...
        if olla_agent.__class__.__name__ == "OuterLoopLinkAdaptation" or "NonLinear_OLLA":
            loss_history = None
        else:
            loss_history = olla_agent.loss_history
        
    if hasattr(olla_agent, "save_q_network"):
        dqn_network_path = "DQN_models/" + olla_agent.__class__.__name__ + "_" + str(trail_index) + ".h5"
        olla_agent.save_q_network(dqn_network_path)
...
def OLLA_simulation(olla_agent, num_trails, num_time_slots, feedback_interval, num_quant_bits, cqi_delay:int, file_name:str, add_interference:bool,
    ...
    for idx_train in tqdm(range(num_trails)):
        ack_list_single_trail, tbs_list_single_trail, sinr_feedback_list, mcs_list_single_trail, sinr_list_single_trail,\
            ...
            olla_simulation_single_trail(olla_agent, num_time_slots, cqi_intervals=feedback_interval, num_quant_bits=num_quant_bits,\
                cqi_delay=cqi_delay, add_interference=add_interference, variant_sinr=variant_sinr)
    ...
    sinr_result_array = np.array(sinr_result_list).astype(float)
    action_result_array = np.array(action_result_list)
    sinr_offset_result_array = np.array(sinr_offset_result_list).astype(float)
    loss_history_result_array = np.array(loss_history_result_list)
    real_sinr_result_array = np.array(real_sinr_result_list).astype(float)
    
    if olla_agent.__class__.__name__ == "OLLA_DQN_agent":
        file_path = "results_v1/" + file_name +".h5"
    elif olla_agent.__class__.__name__ == "OLLA_DQN_agent_simple" or olla_agent.__class__.__name__ == "OuterLoopLinkAdaptation" or \
        olla_agent.__class__.__name__ == "NonLinear_OLLA" or olla_agent.__class__.__name__ == "NonLinear_OLLA_bp":
    ...
    if hasattr(olla_agent, "get_action_list"):
        action_choices = np.array(olla_agent.get_action_list())
        olla_lut_file.create_dataset(name="action_space", data=action_choices)
    if loss_history_result_array[0] != None:     #or loss_history_result_array[0].all() != None 
...
def la_simulation():
    # LA algorithms
    agent_name = "NOLLA"     # OLLA or OLLA_DQN_v1 or OLLA_DQN_v2 or NOLLA or NOLLA_bp
    ack_weight = 0
    simulation_name = "per_UE_june_6666"
    gamma = 0.75
    bler_target = 0.03
    ...
    file_name = generate_file_name(agent_name, num_trails, feedback_interval, num_quant_bits, cqi_delay, ack_weight, add_interference,\
        simulation_name, input_time_dimension, variant_sinr, gamma)
    
    olla_dqn_agent = OLLA_DQN_agent(bler_target=bler_target, 
                                data_file="/home/zhu/Codes/link_adaptation/BLER_LUT_data_simulation/table3_LUT_AWGN_simulation.h5",
                                olla_step_size=0.1,
                                reliablity_weight=ack_weight,
                                num_time_slots=input_time_dimension, 
                                gamma=gamma)
    olla_dqn_agent_simple = OLLA_DQN_agent_simple(bler_target=bler_target,
                                data_file="/home/zhu/Codes/link_adaptation/BLER_LUT_data_simulation/table3_LUT_AWGN_simulation.h5",
                                olla_step_size= 0.1,
                                reliablity_weight=ack_weight,
                                num_time_slots_q_network=input_time_dimension)
    olla_agent = OuterLoopLinkAdaptation(bler_target=bler_target, 
                                data_file="/home/zhu/Codes/link_adaptation/BLER_LUT_data_simulation/table3_LUT_AWGN_simulation.h5",
                                olla_step_size=0.1)
    Nolla_agent = NonLinear_OLLA(bler_target=bler_target,
                                 data_file="/home/zhu/Codes/link_adaptation/BLER_LUT_data_simulation/table3_LUT_AWGN_simulation.h5",
                                 olla_step_size=0.1)
    Nolla_bp_agent = NonLinear_OLLA_bp(bler_target=bler_target,
                                    data_file="/home/zhu/Codes/link_adaptation/BLER_LUT_data_simulation/table3_LUT_AWGN_simulation.h5",
                                    olla_step_size=0.1)
    ...
    elif agent_name == "NOLLA":
        simulation_agent = Nolla_agent
    elif agent_name == "NOLLA_bp":
        simulation_agent = Nolla_bp_agent
        
    
    # simulation_agent.plot_reward_normalized()
    OLLA_simulation(simulation_agent, num_trails, num_time_slots, feedback_interval, num_quant_bits, cqi_delay=cqi_delay, file_name=file_name, add_interference=add_interference, variant_sinr=variant_sinr)
