...
def olla_lut_simulation_perfect_single_trail(num_time_slots:int, cqi_intervals:int, num_quant_bits:int):
...
def olla_LUT_simulation_quant_sinr_feedback(num_trails, num_time_slots, feedback_interval, num_quant_bits):
    ...
    for idx_train in range(num_trails):
        ack_list_single_trail, tbs_list_single_trail, sinr_feedback_list, mcs_list_single_trail, sinr_list_single_trail = \
            olla_lut_simulation_perfect_single_trail(num_time_slots, feedback_interval, num_quant_bits)
    ...
    file_path = "results/new_olla_LUT_sinr_quant_interval_" + str(feedback_interval) + \
        "_quant_bis_" + str(num_quant_bits) +"_interference_result.h5"
...
if __name__ == "__main__":
    olla_LUT_simulation_quant_sinr_feedback(num_trails=100, num_time_slots=500, feedback_interval=5, num_quant_bits=3)
